...
const TrackingPageUs = () => {
    ...
    try {
      ...
      if (res.success && res.orders) {
        ...
        if (currentBooking) {
          ...
          const userLat = parseFloat(currentBooking.userDetails?.latitude || currentBooking.user?.latitude || 0);
          const userLng = parseFloat(currentBooking.userDetails?.longitude || currentBooking.user?.longitude || 0);
  ...
  const userLat = parseFloat(booking?.userDetails?.latitude || booking?.user?.latitude || 0);
  const userLng = parseFloat(booking?.userDetails?.longitude || booking?.user?.longitude || 0);

  const validCoordinates = vendorLat !== 0 && vendorLng !== 0 && !isNaN(vendorLat) && !isNaN(vendorLng) && userLat !== 0 && userLng !== 0;
    ...
    <SafeAreaView style={styles.container}>
      ...
      {validCoordinates ? (
        ...
        <View style={styles.noMapContainer}>
          <Text style={styles.noMapText}>Waiting for User or Vendor Location...</Text>
        </View>
      )}
      ...
      <View style={styles.headerOverlay}>
        ...
        <Text style={styles.headerTitle}>Customer Tracking</Text>
      </View>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.vendorInfoRow}>
            ...
            <View style={styles.vendorTextContainer}>
              <Text style={styles.vendorName}>{booking?.userDetails?.name || booking?.user?.name || 'Customer'}</Text>
              <Text style={styles.vendorPhone}>{booking?.userDetails?.phone || booking?.user?.phone || 'No Phone'}</Text>
            </View>
            <TouchableOpacity 
              style={styles.callBtn}
              onPress={() => {
                const phone = booking?.userDetails?.phone || booking?.user?.phone;
