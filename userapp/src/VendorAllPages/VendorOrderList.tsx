...
const STAGES = [
  { id: 'pending', label: 'Pending', icon: '⏳', desc: 'Awaiting confirmation' },
  { id: 'confirmed', label: 'Confirmed', icon: '📝', desc: 'Confirm availability' },
...
const VendorOrderList = () => {
  ...
  const renderOrderItem = ({ item }: { item: any }) => {
    ...
    return (
      <ImageBackground
        source={require('../assets/vendorOrderBoximage.png')}
        style={styles.orderCard}
        imageStyle={{ borderRadius: 20 }}
      >
        <View style={styles.orderCardOverlay}>
          <View style={styles.orderHeaderRow}>
            <Text style={styles.cardLabel}>ORDER ID: <Text style={styles.cardValue}>#{item.bookingRef || item._id.slice(-6).toUpperCase()}</Text></Text>
          </View>
          <View style={styles.orderHeaderRow}>
            <Text style={styles.cardLabel}>DATE: <Text style={styles.cardValue}>{new Date(item.createdAt).toLocaleDateString()}</Text></Text>
          </View>

          <View style={{ position: 'absolute', top: 20, right: 20, zIndex: 10 }}>
            <TouchableOpacity
              style={[styles.statusBadge, { backgroundColor: 'rgba(0,0,0,0.5)', borderColor: getStatusColor(item.status), borderWidth: 1 }]}
              onPress={() => {
          ...
          <View style={styles.infoSection}>
            <Text style={styles.cardSectionTitle}>CUSTOMER</Text>
            <Text style={styles.cardMainText}>{user.name?.toUpperCase() || 'ANONYMOUS'}</Text>
            <Text style={styles.cardSubText}>{user.phone || 'No phone'} • {user.address || 'No address'}</Text>
          </View>

          <View style={styles.infoSection}>
            <Text style={styles.cardSectionTitle}>VEHICLE</Text>
            <Text style={styles.cardMainText}>
              {vehicle.brand?.toUpperCase()} {vehicle.model?.toUpperCase()} ({vehicle.registration_no || vehicle.number})
            </Text>
            <Text style={styles.cardSubText}>
              {vehicle.vehicle_category?.replace('_', ' ')?.toUpperCase() || 'VEHICLE'} • {vehicle.fuel_type?.toUpperCase() || 'FUEL'}
            </Text>
          </View>
          ...
          <View style={styles.infoSection}>
            ...
            {services.map((s: string, i: number) => (
              <Text key={i} style={styles.cardMainText}>{s.toUpperCase()}</Text>
            ))}
          </View>
          ...
          <View style={styles.footerRow}>
            <View>
              <Text style={styles.cardSectionTitle}>TOTAL BILL :</Text>
              <Text style={styles.totalAmountGreen}>Rs.-{item.totalAmount}</Text>
            </View>
            {updatingId === item._id ? (
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                ...
                <TouchableOpacity
                  style={[
                    styles.actionButton,
                    { backgroundColor: ['delivered', 'completed'].includes(item.status) ? '#555' : '#FFF' }
                  ]}
                ...
                >
                  <Text style={[
                    ...
                    { color: ['delivered', 'completed'].includes(item.status) ? '#AAA' : '#000' }
                  ]}>
                    Track 📍
                  </Text>
                </TouchableOpacity>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabContainer}>
            {STAGES.map((stage) => {
              ...
              return (
                <TouchableOpacity
                  key={stage.id}
                  style={[styles.tab, selectedTab === stage.id && styles.activeTab]}
                  onPress={() => setSelectedTab(stage.id)}
                >
                  <Text style={[styles.tabText, selectedTab === stage.id && styles.activeTabText]}>
                    {stage.label.toUpperCase()} ({count})
                  </Text>
                </TouchableOpacity>
              );
            })}
...
const styles = StyleSheet.create({
  ...
  orderHeaderRow: { marginBottom: 8 },
  cardLabel: { color: '#FFF', fontSize: 13, fontWeight: '800' },
  cardValue: { color: '#CCC', fontWeight: '400' },
  statusBadge: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8 },
  statusText: { fontSize: 10, fontWeight: '900' },
  cardDivider: { height: 1, backgroundColor: 'rgba(255,255,255,0.2)', marginVertical: 15 },
  infoSection: { marginBottom: 15 },
  cardSectionTitle: { color: '#FFF', fontSize: 11, fontWeight: '800', marginBottom: 5, letterSpacing: 0.5 },
  cardMainText: { color: '#FFF', fontSize: 14, fontWeight: '600' },
  cardSubText: { color: '#AAA', fontSize: 12, marginTop: 3 },
  footerRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'flex-end',
  },
  totalAmountGreen: { color: '#28a745', fontSize: 22, fontWeight: 'bold', marginTop: 5 },
  acceptButton: { backgroundColor: '#FFF', paddingHorizontal: 25, paddingVertical: 10, borderRadius: 10, alignSelf: 'flex-end' },
  acceptButtonText: { color: '#000', fontWeight: 'bold', fontSize: 14 },
  actionButton: { paddingHorizontal: 15, paddingVertical: 10, borderRadius: 10 },
  actionButtonText: { fontWeight: 'bold', fontSize: 13, color: '#000' },

  emptyText: { textAlign: 'center', marginTop: 100, color: '#FFF', fontSize: 16 },

  tabScrollContainer: {},
  tabContainer: { paddingHorizontal: 15, paddingVertical: 10, gap: 15 },
  tab: {
    paddingHorizontal: 5,
    paddingVertical: 8,
    borderBottomWidth: 2,
